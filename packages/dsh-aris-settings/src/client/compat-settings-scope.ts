/**
 * rc.6-compatible settings scope for the dsh-aris family group.
 *
 * Ported from dsh-web-ui-settings (WebUiSettingsBinder) under the
 * `arisSettings` service name so the two families coexist without touching
 * each other's binder. The official settings scope answers "unavailable" for
 * every third-party namespace on rc.6 hosts (the apiproxy allowlist is
 * hard-coded), which turns every family plugin card into a read-only
 * explanation. This binder wraps the official scope: when it reports the
 * namespace ready, the wrapper is a pass-through; when it reports
 * unavailable on a loopback connection, a bridge controller takes over and
 * serves the same SettingsScope contract from this package's host-side
 * bridge routes (/api/dsh-aris-settings). Remote browsers (non-loopback)
 * never use the bridge, matching the official process-local policy. Family
 * plugins opt in through ctx.get('arisSettings') without a hard service
 * dependency, so a deployment without this package keeps the previous
 * behavior.
 */
...
import { ARIS_SETTINGS_BRIDGE_PREFIX } from '../protocol.ts'
...
export function createBridgeApi(fetchFn: typeof fetch): BridgeSettingsFace {
  const post = async (path: string, body: unknown): Promise<EnvelopedResult> => {
    try {
      const response = await fetchFn(ARIS_SETTINGS_BRIDGE_PREFIX + path, {
        method: 'POST',
...
export interface ArisSettingsBinderFace {
  bind<S>(spec: SettingsScopeSpec<S>): SettingsScope<S>
}

/**
 * The rc.6 compatibility binder, provided as the arisSettings service. Its
 ...
 */
export class ArisSettingsBinder extends Service {
  constructor(ctx: Context) {
    super(ctx, 'arisSettings')
  }
  ...
  bind<S>(spec: SettingsScopeSpec<S>): SettingsScope<S> {
    ...
    if (!isBinderFace(official)) {
      ...
      throw new Error('arisSettings: the official settingsScope binder is unavailable')
    }
    ...
    const scope = createCompatScope<S>({
    ...
    }, 'dsh-aris-settings: compat scope invalidation')
    return scope
  }
}

/** True when the value exposes the official settings binder's bind() seam. */
function isBinderFace(value: unknown): value is ArisSettingsBinderFace {
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    ...
    arisSettings: ArisSettingsBinder
  }
}
