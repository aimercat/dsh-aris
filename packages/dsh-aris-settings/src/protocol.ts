 ...
 * namespace answers "settings-not-exposed" and third-party plugin cards can
 * only explain the gap. This bridge re-serves the dsh-aris family namespaces
 * through the host settings seam over a same-origin, loopback-only HTTP pair,
 * gated by the user's web_settings_namespaces allowlist from settings.yaml
 * with a built-in family fallback list. On hosts whose apiproxy already
 * exposes the namespaces, the official settings scope stays the primary
 * transport and this bridge never activates.
 */

/** Bridge route prefix (same-origin, loopback-only). */
export const ARIS_SETTINGS_BRIDGE_PREFIX = '/api/dsh-aris-settings'
