/**
 * rc.6 compatibility allowlist for the dsh-aris settings bridge.
 *
 * Ported from dsh-web-ui-settings with the scope narrowed to the dsh-aris
 * family. rc.6 host-apiproxy never reads settings.yaml for a namespace
 * allowlist: its WEB_SETTINGS_NAMESPACES set is hard-coded. Users configure
 ...
 * users spell as package names like dsh-aris — onto the real settings
 * namespaces the plugins register (aris-live2d and future family members).
 * When the key is absent, a built-in family fallback list keeps every family
 * plugin's configuration form visible out of the box. The final allowlist is
 * always intersected with the namespaces actually registered in the host
 * settings seam, so an unknown entry can never surface a form or accept a
 * write.
 */

/** Settings namespaces the dsh-aris family plugins register. */
...
const NAMESPACE_ALIASES: Readonly<Record<string, string | null>> = {
  'dsh-aris': 'aris-live2d',
  'aris-live2d': 'aris-live2d',
  'dsh-aris-settings': null,
  'dsh-aris-all': null,
}
