/**
 * Host-side settings bridge for the dsh-aris family.
 *
 * Ported from dsh-web-ui-settings: serves the dsh-aris family settings
 * namespaces over a same-origin, loopback-only HTTP pair because rc.6
 * host-apiproxy refuses every third-party namespace at the RPC boundary. The
 * handlers ride the host settings seam (ctx.settings), which keeps the
 * official schema validation, revision fencing, persistence, and event
 * emission for free; the bridge only adds the allowlist gate the apiproxy
 * normally provides. Error codes mirror the official RPC codes so the client
 * controller treats refusals exactly like an apiproxy answer.
 */
...
export function makeBridgeRoutes(deps: BridgeDeps): WebRoute[] {
  ...
  return [
    {
      kind: 'exact',
      path: ARIS_SETTINGS_BRIDGE_PREFIX + '/describe',
    ...
    {
      kind: 'exact',
      path: ARIS_SETTINGS_BRIDGE_PREFIX + '/mutate',
      handler: async (req, res) => {
        if (!guard(req, res)) return
